...
function animCanvas(){
  ...
  for(let i=pkts.length-1;i>=0;i--){
    ...
    for(let k=1;k<=3;k++){
      ...
      ctx.fillStyle=`rgba(${p.r},${p.g},${p.b},${.16-k*.04})`;
      ctx.fill();
    }
    ctx.shadowBlur=14;
  ...
  ctx.shadowBlur=0;
  pktT++; if(pktT>12){spawnPkt();pktT=0;}
  waveT++; if(waveT>140){spawnWave();waveT=0;}
